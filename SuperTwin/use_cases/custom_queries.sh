...
function mem_numa_alloc_hit_query() {
    ...
    echo "_node0" >> "${BENCHMARK_RESULTS}/${bench}/$MATRIX_NAME/${measurement}_data"
    ...
    echo "$QUERY" >> "${BENCHMARK_RESULTS}/${bench}/$MATRIX_NAME/queries.txt"
    echo "_node1" >> "${BENCHMARK_RESULTS}/${bench}/$MATRIX_NAME/${measurement}_data"
    echo "$(get_influx_data "${QUERY}")" >> "${BENCHMARK_RESULTS}/${bench}/$MATRIX_NAME/${measurement}_data"
}
...
function kernel_pernode_cpu_idle_query() {
    ...
    echo "" >> "${BENCHMARK_RESULTS}/${bench}/$MATRIX_NAME/${measurement}_data"
