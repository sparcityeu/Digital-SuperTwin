...
def run_roofline(name, freq, l1_size, l2_size, l3_size, inst, isa, precision, num_ld, num_st, threads, interleaved, num_ops, dram_bytes, test_type, verbose, set_freq, vendor, args):
    ...
    if (test_type == 'L1' or test_type == 'roofline'):
        ...
        if(interleaved):
            result = subprocess.run(["./bin/test", "-threads", str(threads), "-freq", str(freq), "-vendor", vendor, "--interleaved"], stdout=subprocess.PIPE)
        else:
            result = subprocess.run(["./bin/test", "-threads", str(threads), "-freq", str(freq), "-vendor", vendor], stdout=subprocess.PIPE)
    ...
    if (test_type == 'FP' or test_type == 'roofline'):
        ...
        if (verbose > 1):
            ...
            print ("CYCLES PER INNER LOOP:", float(out[0])/float(out[1]))
            print ("ISTRUCTION PER CYCLE:", threads*num_fp*factor*float(out[1])/float(out[0]))
            print ("FlOPS PER CYCLE:", threads*num_fp*factor*ops_fp[isa][precision]*float(out[1])/float(out[0]))
...
def main():
    ...
    parser.add_argument('--vendor', nargs='?', type = str, choices=['intel', 'amd'], help='Name of CPU vendor (intel or amd)')
    ...
    parser.add_argument('--name', nargs='?', type = str, help='Name for results file (if not using config file)')
