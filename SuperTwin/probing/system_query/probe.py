...
def choose_info(hostname, system, cache_info, socket_groups, domains, cache_topology, affinity, gpu_info, PMUs, pmprobe):
    ...
    chosen_info['os'] = system.get('system',{}).get('os',{}).get('version',"")
    chosen_info['arch']  =  system.get('system',{}).get('kernel',{}).get("arch","")
    chosen_info['uuid'] = system.get('uuid',"")
    
    chosen_info['system'] = {}
    
    chosen_info['system']['motherboard'] = {}
    chosen_info['system']['motherboard']['name'] = system.get('system',{}).get('motherboard',{}).get('name',"")
    chosen_info['system']['motherboard']['vendor'] = system.get('system',{}).get('motherboard',{}).get('vendor',"")
    
    chosen_info['system']['bios'] = {}
    chosen_info['system']['bios']['version'] = system.get('firmware',{}).get('bios',{}).get('version',"")
    chosen_info['system']['bios']['date'] = system.get('firmware',{}).get('bios',{}).get('date',"")
    chosen_info['system']['bios']['vendor'] = system.get('firmware',{}).get('bios',{}).get('vendor',"")
    
    chosen_info['system']['kernel'] = {}
    chosen_info['system']['kernel']['version'] =  system.get('system',{}).get('kernel',{}).get('version',"")
    
    chosen_info['memory'] = {}
    chosen_info['memory']['total'] = {}
    chosen_info['memory']['total']['size'] = int(system.get('memory',{}).get('total',{}).get('size',0))
    chosen_info['memory']['total']['banks'] = int(system.get('memory',{}).get('total',{}).get('banks',0))
    ...
    for key in system.get('memory',{}):
        if(key.find('bank:') != -1):
            ...
            try:
                temp_bank['size'] = int(system['memory'].get(ident,{}).get('size',0))
                temp_bank['slot'] = system['memory'].get(ident,{}).get('slot',"")
                temp_bank['clock'] = int(system['memory'].get(ident,{}).get('clock',""))
                temp_bank['description'] = system['memory'].get(ident,{}).get('description',"")
                temp_bank['vendor'] = system['memory'].get(ident,{}).get('vendor',"")
                temp_bank['model'] = system['memory'].get(ident,{}).get('product',"")
    ...
    for key in system.get('network',{}):
        ...
        try:
            chosen_info['network'][key]['ipv4'] = system['network'][key].get('ipv4',"")
        except:
            chosen_info['network'][key]['ipv4'] = ''

        try:
            chosen_info['network'][key]['businfo'] = system['network'][key].get('businfo',"")
            chosen_info['network'][key]['vendor'] = system['network'][key].get('vendor',"")
            chosen_info['network'][key]['model'] = system['network'][key].get('product',"")
            chosen_info['network'][key]['firmware'] = system['network'][key].get('firmware',"")
        ...
        try:
            chosen_info['network'][key]['speed'] = system['network'][key].get('speed',"")
        except:
            chosen_info['network'][key]['speed'] = 'no-link'
            
        chosen_info['network'][key]['serial'] = system['network'][key].get('serial',"")
        chosen_info['network'][key]['link'] = system['network'][key].get('link',"")
    ...
    chosen_info['cpu']['specs']['sockets'] = int(system.get('cpu',{}).get('physical',{}).get('number',0))
    ##From there, assumes all cpus will be identical on the same machine
    chosen_info['cpu']['specs']['model'] = system.get('cpu',{}).get('physical_0',{}).get('product',"")
    chosen_info['cpu']['specs']['type'] = system.get('cpu',{}).get('physical_0',{}).get('architecture',"")
    chosen_info['cpu']['specs']['cores'] = int(system.get('cpu',{}).get('physical_0',{}).get('cores',0)) 
    chosen_info['cpu']['specs']['threads'] = int(system.get('cpu',{}).get('physical_0',{}).get('threads',0)) 
    chosen_info['cpu']['specs']['threads_per_core'] = int(system.get('cpu',{}).get('physical_0',{}).get('threads_per_core',0))
    chosen_info['cpu']['specs']['hyperthreading'] = system.get('cpu',{}).get('physical',{}).get('smt',"")
    chosen_info['cpu']['specs']['min_mhz'] = system.get('cpu',{}).get('physical_0',{}).get('min_Mhz',"")
    chosen_info['cpu']['specs']['max_mhz'] = system.get('cpu',{}).get('physical_0',{}).get('max_Mhz',"")
    #chosen_info['cpu']['specs']['bus_mhz'] = system['cpu']['physical_0']['bus_mhz']
    chosen_info['cpu']['specs']['flags'] = system.get('cpu',{}).get('physical_0',{}).get('flags',"")
