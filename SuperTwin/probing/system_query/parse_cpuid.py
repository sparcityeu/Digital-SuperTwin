...
def parse_cpuid():
    ...
    if not faulty:
        info['cache']['L1D']['size'] = gv_parentheses_space(
            cache_0[16]) if 16 < len(cache_0) else 0
    ...
    if not faulty:
        info['cache']['L1I']['size'] = gv_parentheses_space(
            cache_1[16]) if 16 < len(cache_1) else 0
    ...
    if not faulty:
        info['cache']['L2']['size'] = gv_parentheses_space(
            cache_2[16]) if 16 < len(cache_2) else 0
    ...
    if not faulty:
        info['cache']['L3']['size'] = gv_parentheses_space(
            cache_3[16]) if 16 < len(cache_3) else 0
    ...
    info['monitoring']['bit_width_of_fixed_pmcs'] = gv_parentheses(
        monitoring[16])
    ...
    if(faulty):
        # On some very old architectures, CPUID doesn't work as expected
        info = fix_faulty_report(info, name)
