...
def parse_likwid():
    ...
    for i in range(no_sockets):
        ...
        socket_list = topol[find_ind(to_find, topol)].split('\t')[
            2].strip('(').strip(')').split(' ')
        socket_list.remove('')
        socket_list.remove('')
        ...
        # Domains
        domains[str(i)] = {}
        # if does not work in general, remove \t until none left
        to_find = 'Domain:\t\t\t' + str(i)
        start = find_ind(to_find, topol)
        processor_list = topol[start +
                               1].split('\t')[2].strip('(').strip(')').split(' ')
...
def parse_affinity():
    ...
    cores_per_socket = int(remove_whitespace(
        num_info[find_ind("Cores per socket:", num_info)].split("\t"))[1])
    threads_per_core = int(remove_whitespace(
        num_info[find_ind("Threads per core:", num_info)].split("\t"))[1])
