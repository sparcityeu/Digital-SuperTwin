...
def one_size(metrics, alias):
    ...
    linux_datapoints = s_20.loc[s_20["component"] == "pmdalinux"]
    linux_datapoints  = int(list(linux_datapoints["br_datapoints"])[0])

    proc_datapoints = s_20.loc[s_20["component"] == "pmdaproc"]
    proc_datapoints  = int(list(proc_datapoints["br_datapoints"])[0])

    perfevent_datapoints = s_20.loc[s_20["component"] == "pmdaperfevent"]
    perfevent_datapoints  = int(list(perfevent_datapoints["br_datapoints"])[0])

    
    trace10 = go.Bar(name="perfevent",
                     x=["All"],
                     y=[perfevent_datapoints],
                     marker_color = colors_4[1],
                     showlegend=False)

    
    trace11 = go.Bar(name="proc",
                    x=["All"],
                    y=[proc_datapoints],
                     marker_color = colors_4[3],
                    showlegend=False)
    ...
    base = perfevent_datapoints + proc_datapoints
    trace12 = go.Bar(name="linux",
                     x=["All"],
                     y=[linux_datapoints],
                     marker_color = colors_4[2],
                     showlegend=False)
    ...
    fig = make_subplots(rows=1, cols=4, column_widths=[0.7, 0.1, 0.1, 0.1], horizontal_spacing=0.075, shared_xaxes=True)
    ...
    fig.append_trace(trace9, 1,4)
    fig.append_trace(trace10, 1,3)
    ...
    fig.layout['yaxis3'].update(title="Reported Datapoints")
    fig.layout['yaxis3'].update(type="log", dtick=0.30102999566, title="Reported Datapoints", tickvals=[256,1024,2048,4096,8192])
    fig.layout['yaxis4'].update(title="Network Traffic [MB/s]")
