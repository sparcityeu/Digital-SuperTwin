...
colors_4 = ["rgb(240,210,126)", "rgb(139,18,58)", "rgb(212,212,212)", "rgb(208,100,64)", "rgb(93,62,71)"]
...
def one_size(metrics, alias):
    ...
    trace6 = go.Bar(name="perfeventmem",
                     x=xx,
                     y=perfevent_mem,
                     offsetgroup=0,
                     base=pmcd_mem,
    ...
    fig = make_subplots(rows=1, cols=3, column_widths=[0.7, 0.2, 0.1], horizontal_spacing=0.1, shared_xaxes=True)
    ...
    fig.update_traces(texttemplate='%{y:d}', textposition='auto', selector = ({'name':'pmcdmem'}))
    fig.update_traces(texttemplate='%{y:d}', textposition='auto', selector = ({'name':'perfeventmem'}))
    fig.update_traces(texttemplate='%{y:d}', textposition='auto', selector = ({'name':'linuxmem'}))
    fig.update_traces(texttemplate='%{y:.2f}', textposition='auto', selector = ({'name':'procmem'}))
    ...
    fig.update_layout(
        title="Poseidon, " + metrics + " metrics " + str(datapoints[0]) + " datapoints",
        font=dict(
...
if __name__ == "__main__":
    ...
    one_size("10" ,"poseidon10")
    one_size("20" ,"poseidon20")
    one_size("30" ,"poseidon30")
    one_size("40" ,"poseidon40")
    one_size("50" ,"poseidon50")
