...
def one_size(metrics, alias):
    ...
    linux_mem_l = s_20.loc[s_20["component"] == "pmdalinux"]
    
    proc_mem_l = s_20.loc[s_20["component"] == "pmdaproc"]

    proc_mem = list(linux_mem_l["mem_use_mean"])
    linux_mem = list(proc_mem_l["mem_use_mean"])
    ...
    fig.update_traces(texttemplate='%{y:.2f}', textposition='auto', selector = ({'name':'pmcdmem'}))
    fig.update_traces(texttemplate='%{y:.2f}', textposition='auto', selector = ({'name':'perfeventmem'}))
    fig.update_traces(texttemplate='%{y:.2f}', textposition='auto', selector = ({'name':'linuxmem'}))
