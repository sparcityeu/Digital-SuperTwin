...
def one_run_sampled(SuperTwin, bench, config, db, baseline, no_metrics, frequency):
    ...
    for i in range(repeat):
        ...
        if("perfevent_hwcounters_UOPS_RETIRED_SLOTS_value" in involved):
            uops_retired_any, p, zp = get_sum("perfevent_hwcounters_UOPS_RETIRED_SLOTS_value")
...
if __name__ == "__main__":
    ...
    for size in sizes:
        for bench in benchs:
            ...
            config = "-t 1 -c cstress_configs/poseidon_" + str_mtr + ".conf :configured"
            one_run_sampled(my_superTwin, bench, config, "poseidon_run", baseline, mtr, 1)
            print("##################" + bench + "##################")
            
            config = "-t 0.5 -c cstress_configs/poseidon_" + str_mtr + ".conf :configured"
            one_run_sampled(my_superTwin, bench, config, "poseidon_run", baseline, mtr, 2)
            print("##################" + bench + "##################")
            
            config = "-t 0.25 -c cstress_configs/poseidon_" + str_mtr + ".conf :configured"
            one_run_sampled(my_superTwin, bench, config, "poseidon_run", baseline, mtr, 4)
            print("##################" + bench + "##################")
            
            config = "-t 0.125 -c cstress_configs/poseidon_" + str_mtr + ".conf :configured"
            one_run_sampled(my_superTwin, bench, config, "poseidon_run", baseline, mtr, 8)
            print("##################" + bench + "##################")
            
            config = "-t 0.0625 -c cstress_configs/poseidon_" + str_mtr + ".conf :configured"
