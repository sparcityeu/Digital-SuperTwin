        ...
        for content in td[interface]["contents"]:
            if content["@type"].find("Telemetry") != -1:
                if content["DBName"] == measurement:
                    params.append(
                        {
                            "Alias": td[interface]["displayName"],
                            "Param": content["displayName"],
                        }
                    )
...
def get_params_interface_known(td, interface, measurement):
    ...
    if measurement == "hinv_cpu_clock":  ##Small glitch in twin generation
        params = {
            "Alias": td[interface]["displayName"],
            "Param": td[interface]["contents"][0]["displayName"],
        }
        ...
        if interface.find("socket") != -1:
            ...
            for content in td[interface]["contents"]:
                if (
                    ...
                    and content["target"].find("core") != -1
                ):
            ...
            for core in cores:
                ...
                for content in _interface["contents"]:
                    if (
                        content["@type"] == "Relationship"
                        and content["target"].find("thread") != -1
                    ):
...
def generate_monitoring_dashboard(SuperTwin):
    ...
    empty_dash["panels"].append(
        mp.name_panel_html(datasource, get_next_id(), SuperTwin.name)
    )
    ...
    for param in params:
        empty_dash["panels"][panel_id]["targets"].append(
            mp.stat_query(
                datasource,
                param["Alias"],
                "mem_numa_alloc_hit",
    ...
    empty_dash["panels"].append(
        mp.stat_panel(
            datasource, panel_id, 5, 4, 20, 0, "continuous-GrYlRd", "Numa Load"
        )
    )
    ...
    for idx, param in enumerate(params):
        empty_dash["panels"][panel_id]["targets"].append(
            mp.stat_query(
                datasource,
                param["Alias"],
                "kernel_pernode_cpu_idle",
                param["Param"],
            )
        )
        empty_dash["panels"][panel_id]["targets"][idx]["select"][0].append(
            {"params": [" / 440 * -1 + 100"], "type": "math"}
        )
    ...
    try:
        params = [
            params[1],
            params[panel_id],
        ]  ##small glitch in twin generation
        # print("Params:", params)
        for idx, param in enumerate(params):
            param["Param"] = param["Param"].replace(
                "core", "cpu"
            )  ##Small glitch in twin generation
            empty_dash["panels"][panel_id]["targets"].append(
                mp.stat_query(
                    datasource,
                    param["Alias"],
                    "perfevent_hwcounters_RAPL_ENERGY_PKG_value",
                    param["Param"],
                )
            )
            empty_dash["panels"][panel_id]["targets"][idx]["select"][0].append(
                {"params": ["* 0.23 * 0.000000001"], "type": "math"}
            )
            empty_dash["panels"][panel_id]["fieldConfig"]["defaults"][
                "unit"
            ] = "watt"
    except:
        print(
            "RAPL driver can't be accessed for target host. Proceed without power measurement.."
        )
        ...
        for idx, param in enumerate(params):
            ...
            empty_dash["panels"][panel_id]["targets"].append(
                mp.stat_query(
                    datasource,
                    param["Alias"],
                    "perfevent_hwcounters_RAPL_ENERGY_DRAM_value",
                    param["Param"],
                )
            )
            empty_dash["panels"][panel_id]["targets"][idx]["select"][0].append(
    ...
    params = get_params(td, "mem_numa_alloc_other_node")
    for param in params:
        empty_dash["panels"][6]["targets"].append(
            mp.stat_query(
                datasource,
                param["Alias"],
                "mem_numa_alloc_local_node",
                param["Param"],
            )
        )
    ...
    for idx, param in enumerate(params):
        empty_dash["panels"][8]["targets"].append(
            mp.stat_query(
                datasource,
                param["Alias"],
                "mem_numa_util_used",
        ...
        empty_dash["panels"][8]["targets"][idx]["select"][0].append(
            {"params": [" / 1048576"], "type": "math"}
        )
        empty_dash["panels"][8]["fieldConfig"]["defaults"][
            "unit"
        ] = "decgbytes"
    ...
    for idx, socket in enumerate(topology):
        ...
        empty_dash["panels"].append(
            mp.clock_panel(
                ...
                4 + ((idx) * 6),
                0,
                color_schemes_clock[idx],
                "Thread Frequency - Socket " + str(idx),
            )
        )
    ...
    for idx2, socket in enumerate(topology):
        ...
        empty_dash["panels"].append(
            mp.clock_panel(
                ...
                7 + idx2 * 6,
                0,
                color_schemes_load[idx2],
                "Thread Load - Socket " + str(idx2),
            )
        )
        # print("no_panels:", len(empty_dash))
        for idt, thread in enumerate(
            topology[socket]
        ):  ##Note that, that was a single list
            param = get_params_interface_known(
                td, thread, "kernel_percpu_cpu_idle"
            )
            empty_dash["panels"][panel_id]["targets"].append(
                mp.clock_query(
                    datasource,
            ...
            empty_dash["panels"][panel_id]["targets"][idt]["select"][0].append(
                {"params": [" /10 *-1 +100"], "type": "math"}
            )

    ##no_process
    panel_id = get_next_id()
    empty_dash["panels"].append(
        mp.small_single_timeseries(
            datasource, panel_id, 5, 4, 0, 5, "# of processes"
        )
    )
    empty_dash["panels"][panel_id]["targets"].append(
        mp.small_single_query(
            datasource, "# of processes", "kernel_all_nprocs"
        )
    )
    ...
    empty_dash["panels"].append(
        mp.small_single_timeseries(
            datasource, panel_id, 5, 4, 0, 10, "# of users"
        )
    )
    empty_dash["panels"][panel_id]["targets"].append(
        mp.small_single_query(datasource, "# of users", "kernel_all_nusers")
    )

    ##network top
    empty_dash["panels"].append(
        mp.all_network_panel(datasource, get_next_id(), 12, 6, 0, 15)
    )

    ##disk reads
    panel_id = get_next_id()
    disk_reads = empty_dash["panels"].append(
        mp.disk_panel(datasource, panel_id, 12, 5, 6, 15, "Disk Reads")
    )
    params = get_params(td, "disk_dev_read")
    for dev in params:
        empty_dash["panels"][panel_id]["targets"].append(
            mp.stat_query(
                datasource, dev["Alias"], "disk_dev_read", dev["Param"]
            )
        )

    ##disk writes
    panel_id = get_next_id()
    disk_writes = empty_dash["panels"].append(
        mp.disk_panel(datasource, panel_id, 12, 5, 11, 15, "Disk Writes")
    )
    params = get_params(td, "disk_dev_write")
    for dev in params:
        empty_dash["panels"][panel_id]["targets"].append(
            mp.stat_query(
                datasource, dev["Alias"], "disk_dev_write", dev["Param"]
            )
        )
