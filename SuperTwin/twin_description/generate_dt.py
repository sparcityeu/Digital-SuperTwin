...
def get_property(_id, name, schema="string", displayname="", description=""):
    ...
    _property[
        "name"
    ] = name  # Note that this name is different than displayName
    _property[
        "schema"
    ] = schema  # In dtdl v2, properties require name field and schema
...
def get_telemetry(
    hostname, name, displayname, comp, version, description="db pointer"
):
...
def get_telemetry_mapped(
    hostname, name, field_key, measurement, comp, version
):
...
def get_supertwin_telemetry_mapped(
    hostname, name, field_key, measurement, comp, version
):
    ...
    st_telemetry["DBName"] = (
        measurement[0].replace(
            "perfevent.hwcounters.", "perfevent_hwcounters_"
        )
        + "_value"
    )
    st_telemetry["PMUName"] = measurement[1]
    st_telemetry["description"] = measurement[2]
    st_telemetry["pmu_group"] = measurement[3]
...
def _filter(param_metric):
    ...
    if f_metric.find("percpu") != -1:
        ...
        f_metric.find("perfevent.hwcounters") != -1
...
def add_my_metrics(
    models_dict, this_comp_id, hostname, displayname, my_categories
):
    ...
    for count, metric in enumerate(my_metrics):
        ...
        models_dict[this_comp_id]["contents"].append(
            get_telemetry(hostname, m_name, metric, displayname, 1)
        )

    return models_dict


def add_my_metrics_mapped(
    models_dict, this_comp_id, hostname, displayname, field_key, my_categories
):
    ...
    for count, my_metric in enumerate(my_metrics):
        ...
        if type(my_metric) == list:
            models_dict[this_comp_id]["contents"].append(
                get_supertwin_telemetry_mapped(
                    hostname, m_name, field_key, measurement, displayname, 1
                )
            )
        else:
            models_dict[this_comp_id]["contents"].append(
                get_telemetry_mapped(
                    hostname, m_name, field_key, measurement, displayname, 1
                )
            )
...
def add_my_metrics_mapped_socket(
    models_dict,
    this_comp_id,
    hostname,
    displayname,
...
def add_sockets(models_dict, _sys_dict, top_id, hostname, socket):
    ...
    field_key_hw = "_core" + str(
        _sys_dict["numa"][str(socket)]["processors"][0]
    )
    ...
    models_dict[top_id]["contents"].append(
        get_relationship(
            get_uid(hostname, "system", "ownership" + contains, 1),
            "contains" + contains,
            this_socket_id,
        )
    )
    ...
    this_socket["contents"].append(
        get_property(
            ...
            "model",
            description=_sys_dict["cpu"]["specs"]["model"],
        )
    )

    this_socket["contents"].append(
        get_property(
            get_uid(hostname, displayName, "property1", 1),
            "cores",
            description=_sys_dict["cpu"]["specs"]["cores"],
        )
    )

    this_socket["contents"].append(
        get_property(
            get_uid(hostname, displayName, "property2", 1),
            "threads",
            description=_sys_dict["cpu"]["specs"]["threads"],
        )
    )

    this_socket["contents"].append(
        get_property(
            get_uid(hostname, displayName, "property3", 1),
            "threads_per_core",
            description=_sys_dict["cpu"]["specs"]["threads_per_core"],
        )
    )

    this_socket["contents"].append(
        get_property(
            get_uid(hostname, displayName, "property4", 1),
            "hyperthreading",
            description=_sys_dict["cpu"]["specs"]["hyperthreading"],
        )
    )

    this_socket["contents"].append(
        get_property(
            get_uid(hostname, displayName, "property5", 1),
            "max_mhz",
            description=_sys_dict["cpu"]["specs"]["max_mhz"],
        )
    )

    this_socket["contents"].append(
        get_property(
            get_uid(hostname, displayName, "property6", 1),
            "min_mhz",
            description=_sys_dict["cpu"]["specs"]["min_mhz"],
        )
    )

    this_socket["contents"].append(
        get_property(
            get_uid(hostname, displayName, "property7", 1),
            "flags",
            description=_sys_dict["cpu"]["specs"]["flags"],
        )
    )
...
def add_cores(
    models_dict, _sys_dict, top_id, hostname, socket_id, socket_num, core
):
    ...
    models_dict[socket_id]["contents"].append(
        get_relationship(
            get_uid(hostname, socket_displayName, "contains" + contains, 1),
            "contains" + contains,
            this_core_id,
        )
    )
...
def add_threads(
    ...
    models_dict[core_id]["contents"].append(
        get_relationship(
            get_uid(hostname, core_displayName, "contains" + contains, 1),
            "contains" + contains,
            this_thread_id,
        )
    )
    ...
    for group in _sys_dict["cpu"]["cache"][cache]["cache_groups"]:
        ...
        models_dict[cache_id]["contents"].append(
            get_property(
                ...
                "associativity",
                description=_sys_dict["cpu"]["cache"][cache]["associativity"],
            )
        )
        models_dict[cache_id]["contents"].append(
            get_property(
                get_uid(hostname, _added, "property1", 1),
                "cache_line_size",
                description=_sys_dict["cpu"]["cache"][cache][
                    "cache_line_size"
                ],
            )
        )
        models_dict[cache_id]["contents"].append(
            get_property(
                get_uid(hostname, _added, "property2", 1),
                "no_sets",
                description=_sys_dict["cpu"]["cache"][cache]["no_sets"],
            )
        )
        models_dict[cache_id]["contents"].append(
            get_property(
                get_uid(hostname, _added, "property3", 1),
                "size",
        ...
        for thread in group:
            ...
            contains = c()
            models_dict[that_thread_id]["contents"].append(
                get_relationship(
                    get_uid(
                        hostname,
                        that_thread_displayName,
                        "contains" + contains,
                        1,
                    ),
                    "contains" + contains,
...
def add_cpus(models_dict, _sys_dict, top_id, hostname):
    ...
    for socket in _sys_dict["affinity"]["socket"]:
        ...
        models_dict, this_socket_id = add_sockets(
            models_dict, _sys_dict, top_id, hostname, socket
        )

        for core in _sys_dict["affinity"]["socket"][socket]["cores"]:

            models_dict, this_core_id = add_cores(
                models_dict,
                _sys_dict,
                top_id,
                hostname,
            ...
            for thread in _sys_dict["affinity"]["socket"][socket]["cores"][
                core
            ]:

                models_dict, this_thread_id = add_threads(
                    models_dict,
                    _sys_dict,
                    top_id,
                    hostname,
...
def add_memory_banks(models_dict, _sys_dict, top_id, hostname, top_memory):
    ...
    for bank in _sys_dict["memory"]["banks"]:
        ...
        models_dict[top_memory]["contents"].append(
            get_relationship(
                get_uid(hostname, "memory", "contains" + contains, 1),
                "contains" + contains,
                this_bank_id,
            )
        )
        ...
        this_bank["contents"].append(
            get_property(
                get_uid(hostname, displayName, "property0", 1),
                "id",
                description=str(_sys_dict["memory"]["banks"][bank]["id"]),
            )
        )
        this_bank["contents"].append(
            get_property(
                get_uid(hostname, displayName, "property1", 1),
                "size",
                description=str(_sys_dict["memory"]["banks"][bank]["size"]),
            )
        )
        this_bank["contents"].append(
            get_property(
                get_uid(hostname, displayName, "property2", 1),
                "slot",
                description=str(_sys_dict["memory"]["banks"][bank]["slot"]),
            )
        )
        this_bank["contents"].append(
            get_property(
                get_uid(hostname, displayName, "property3", 1),
                "clock",
                description=str(_sys_dict["memory"]["banks"][bank]["clock"]),
            )
        )
        this_bank["contents"].append(
            get_property(
                ...
                "description",
                description=str(
                    _sys_dict["memory"]["banks"][bank]["description"]
                ),
            )
        )
        this_bank["contents"].append(
            get_property(
                get_uid(hostname, displayName, "property6", 1),
                "vendor",
                description=str(_sys_dict["memory"]["banks"][bank]["vendor"]),
            )
        )
        this_bank["contents"].append(
            get_property(
...
def add_memory(models_dict, _sys_dict, top_id, hostname):
    ...
    top_memory["contents"].append(
        get_property(
            get_uid(hostname, displayName, "property0", 1),
            "totalsize",
            description=str(_sys_dict["memory"]["total"]["size"]),
        )
    )
    top_memory["contents"].append(
        get_property(
...
def add_phy_disks(models_dict, _sys_dict, top_id, hostname, top_disk_id):
    ...
    for disk in _sys_dict["disk"]:
        ...
        if type(_sys_dict["disk"][disk]) is dict:
            ...
            this_disk = get_interface(
                this_disk_id, displayname=disk_displayName
            )
            ...
            models_dict[top_disk_id]["contents"].append(
                get_relationship(
                    get_uid(
                        hostname, disk_displayName, "contains" + contains, 1
                    ),
                    "contains" + contains,
                    this_disk_id,
                )
            ...
            this_disk["contents"].append(
                get_property(
                    ...
                    "size",
                    description=str(_sys_dict["disk"][disk]["size"]),
                )
            )
            this_disk["contents"].append(
                get_property(
                    get_uid(hostname, disk_displayName, "property1", 1),
                    "model",
...
def add_subnets(models_dict, _sys_dict, top_id, hostname, top_network_id):
    ...
    for network_key in _sys_dict["network"]:
        ...
        if network["link"] == "yes" and network["virtual"] == "no":
            ...
            this_network_id = get_uid(
                hostname, "network:" + network_displayName, "", 1
            )
            this_network = get_interface(
                this_network_id, displayname=network_displayName
            )
            ...
            models_dict[top_network_id]["contents"].append(
                get_relationship(
                    get_uid(
                        hostname, network_displayName, "contains" + contains, 1
                    ),
                    "contains" + contains,
                    this_network_id,
                )
            ...
            this_network["contents"].append(
                get_property(
                    ...
                    "ipv4",
                    description=str(network["ipv4"]),
                )
            )
            this_network["contents"].append(
                get_property(
                    get_uid(hostname, network_displayName, "property1", 1),
                    "businfo",
                    description=str(network["businfo"]),
                )
            )
            this_network["contents"].append(
                get_property(
                    get_uid(hostname, network_displayName, "property2", 1),
                    "vendor",
                    description=str(network["vendor"]),
                )
            )
            this_network["contents"].append(
                get_property(
                    get_uid(hostname, network_displayName, "property3", 1),
                    "model",
                    description=str(network["model"]),
                )
            )
            this_network["contents"].append(
                get_property(
                    get_uid(hostname, network_displayName, "property4", 1),
                    "firmware",
                    description=str(network["firmware"]),
                )
            )
            this_network["contents"].append(
                get_property(
                    get_uid(hostname, network_displayName, "property5", 1),
                    "virtual",
                    description=str(network["virtual"]),
                )
            )
            this_network["contents"].append(
                get_property(
                    get_uid(hostname, network_displayName, "property6", 1),
                    "speed",
                    description=str(network["speed"]),
                )
            )
            this_network["contents"].append(
                get_property(
                    get_uid(hostname, network_displayName, "property7", 1),
                    "serial",
                    description=str(network["serial"]),
                )
            )
            this_network["contents"].append(
                get_property(
                    get_uid(hostname, network_displayName, "property8", 1),
                    "link",
...
def add_pcp(models_dict, hostname, _sys_dict, top_id, pcp_pids):
    ...
    field_key = (
        '"'
        + pcp_pids["pmdalmsensors"]
        + " /var/lib/pcp/pmdas/lmsensors/pmdalmsensors"
        + '"'
    )
    component_id = get_uid(hostname, displayName, "", 1)
    component = get_interface(component_id, displayname=displayName)
    ...
    field_key = (
        '"'
        + pcp_pids["pmdaperfevent"]
        + " /var/lib/pcp/pmdas/perfevent/pmdaperfevent"
        + '"'
    )
    component_id = get_uid(hostname, displayName, "", 1)
    component = get_interface(component_id, displayname=displayName)
        ...
        if key.find("perf") == -1:
            if should_add(added, key):
                for event in PMUs[key]["events"]:
                    ...
                    metrics.append([metric, event[0], event[1], key])
                added.append(key)
...
def main(_sys_dict, alias, SSHuser, SSHpass, addr):
    ...
    models_dict[top_id]["contents"].append(
        get_property(
            ...
            "os",
            description=_sys_dict["os"],
        )
    )
    models_dict[top_id]["contents"].append(
        get_property(
            get_id(hostname, "arch", 1, "A", 1),
            "arch",
            description=_sys_dict["arch"],
        )
    )
    models_dict[top_id]["contents"].append(
        get_property(
            get_id(hostname, "kernel", 1, "K", 1),
            "kernel",
            description=_sys_dict["system"]["kernel"]["version"],
        )
    )
    ...
    models_dict[top_id]["contents"].append(
        get_property(
            get_id(hostname, "MSR", 1, "M", 1), "MSR", description=msr
        )
    )
