...
def perform_triad(ssh, SuperTwin):
    ...
    stdin, stdout, stderr = ssh.exec_command("likwid-bench -t triad -w S0:1MB:1 -s 30")
...
def one_run_sampled(SuperTwin, bench, config, db):
    ...
    for i in range(repeat):
        ...
        stdin, stdout, stderr = ssh.exec_command("likwid-bench -t copy -w S0:1MB:1 -s 30")
        stdout = stdout.readlines()
...
def write_to_file(times, bench, interval, no_metrics):
    ...
    line = "deren," + str(interval) + "," + str(no_metrics) + "," + bench + "," + str(time_mean) + "," + "time_std"
    writer.close()
...
if __name__ == "__main__":
    ...
    metrics = [" -c cstress/deren_1.conf",
               " -c cstress/deren_2.conf",
               " -c cstress/deren_4.conf"
    ...
    write_to_file(times, bench, interval, -1)
    times = one_run(my_superTwin, "sum")
    write_to_file(times, bench, time, -1)
